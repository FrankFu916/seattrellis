...
/// The RuleSpec registry consumed by the React UI (M3-01/M6-02): the TS
/// side never hard-codes rule lists.
const RULES_TS_OUT: &str = "clients/web/src/api/ruleRegistry.generated.ts";
...
/// The RuleSpec registry as a TS module: typed rule metadata the workbench
/// renders controls from (drift-checked with the committed artifact).
fn rules_typescript() -> String {
    let registry = seattrellis_rules::rule_registry_json();
    let pretty = serde_json::to_string_pretty(&registry).expect("registry serializes");
    format!(
        "// Generated by `cargo run -p xtask -- contract rules` — DO NOT EDIT.\n\
         ...
         export const RULE_SPECS: RuleSpec[] = {pretty};\n"
    )
}
...
fn main() -> ExitCode {
    ...
    match command.as_str() {
        "contract" => match args.get(1).map(String::as_str) {
            ...
            Some("rules") => {
                let content = rules_typescript();
                match write_artifact(RULES_TS_OUT, &content) {
                    Ok(()) => {
                        println!("wrote {RULES_TS_OUT}");
                        ExitCode::SUCCESS
                    }
                    Err(message) => {
                        eprintln!("{message}");
