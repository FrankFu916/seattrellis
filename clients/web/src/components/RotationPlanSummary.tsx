...
export function RotationPlanSummary({
  plan,
  t,
  activePeriod = 1,
  onPeriodSelect,
}: RotationPlanSummaryProps) {
    ...
    <section className="rotation-plan-summary" data-testid="rotation-plan-summary">
      ...
      <ol className="rotation-period-list">
        {plan.periods.map((period) => {
          const isActive = period.period === activePeriod;
          return (
            <li key={period.period}>
              <button
                type="button"
                className={isActive ? "active" : undefined}
                data-testid={`rotation-period-${period.period}`}
                aria-pressed={isActive}
                onClick={() => onPeriodSelect?.(period.period)}
                disabled={!onPeriodSelect}
                title={t("rotation.openPeriod", { period: period.period })}
              >
                <strong>
                  {t("rotation.period", {
