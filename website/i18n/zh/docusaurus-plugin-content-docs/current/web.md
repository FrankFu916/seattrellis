...
- **结果校验**：生成方案后检查硬性约束，并提供各维度评分明细。耗时取决于班级规模和规则复杂度，不保证全局最优。
- **任意两方案对比**：左右下拉框独立选择方案，点击“选用”后才切换正在编辑的方案。预览保留自定义座位编号和异形教室坐标；与自身比较显示零差异，仍可随时选择其他方案。
- **学生调座明细**：展开列表查看每位学生在两方案中的座位，支持按姓名、学号或座位搜索，也可显示未调座的学生。以学号匹配身份，重名学生不会混淆。编辑、撤销或重做后，候选预览和评分审计同步更新；推荐标记和生成时分数仍指原始求解结果。
- **座位图直观互换**：
