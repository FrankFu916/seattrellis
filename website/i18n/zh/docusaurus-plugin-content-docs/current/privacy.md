...
名单导入草稿默认在 2 小时无访问后淘汰；排座编辑草稿、原始求解请求和可视化布局草稿均有容量上限。工作台在切换班级、名单或教室、生成替代方案成功和组件卸载时，请求删除编辑状态与对应的原始求解请求。若旧班级的生成结果延迟返回，也会清理该结果的所有草稿，包括全部轮次。浏览器被关闭或连接中断时，删除请求可能无法完成，因此不承诺安全擦除；本地进程退出后，剩余的未保存内存草稿会随之消失。
