...
欢迎使用 **席序（SeatTrellis）v2.1.0**！本指南将带您在 5 分钟内完成安装，并体验从名单校验、智能排座到最终导出座次表的全流程。
...
- 📐 **[排座规则手册](rules.md)**：深入了解所有硬约束、软偏好加权与算法逻辑。
- 🖥️ **[Web 与桌面工作台指南](web.md)**：掌握可视化界面、多期轮换与排版导出的全部操作。
- 📄 **[数据格式参考](input-format.md)**：查看学生名单、教室布局与快照文件的详细字段规范。
