...
Web 适配器进一步分为三类：`web/workflow.py` 提供不依赖 Streamlit 的文件和服务
适配；`web/components.py` 生成纯数据或 HTML；`web/interactive_panels.py` 只负责
人工编辑、撤销/重做和局部修复等有状态控件。`web/app.py` 负责页面导航、输入材料化
