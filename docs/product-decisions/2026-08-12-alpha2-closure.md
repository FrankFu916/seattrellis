...
| §8.3 条件 | 状态 | 证据 |
|---|---|---|
| 1. 无 `PYTHON_ONLY` / `RUST_PARTIAL` 的 v2 必须项 | **达成** | ledger 200 行：`RUST_VERIFIED=71`。§19.32 判定的 8 个必须项全部关闭：rotation-plan schema_version（§19.34）、CLI 退出码/分类/repair 双形状（§19.33）、pair-report lookback（§19.33）、artifacts compare/restore 契约测试 + containment 修复（§19.35，21 项全绿）、suggest_roster_mapping 启发式差分（§19.36，10 case 全等）。剩余 `RUST_PARTIAL=16` 全部为选项级/非必须项 |
...
| 项 | 类型 | 证据路径 |
|---|---|---|
| ~~artifacts compare/restore 契约（5 行）~~ | **已关闭** | §19.35：21 项 server 契约测试全绿 + M1-05 workspace containment 修复；登记分歧：`restored_at` 形状、candidate_set kind 保留、错误 envelope（M5 待产品） |
...
- 技术线 alpha.2 全部关闭：ledger `RUST_VERIFIED=71`（本轮 +30），
