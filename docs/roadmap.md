...
- 从已保存的轮换方案重新载入编辑器：当前 React 工作台已经可以把所有期次的
  snapshot、锁定状态、来源和操作日志保存为新的项目输出；下一步要支持从该输出
  直接恢复为可继续调整的逐期编辑草稿，而不是只能在历史/比较视图中查看；
...
React 工作台可以把当前多期编辑和操作记录保存为新的轮换计划，但从历史轮换计划
恢复为可继续编辑草稿仍待补齐。
...
- 一次生成多个未来时段的安排；
- 汇总跨期座位公平性、同桌和邻座重复情况；
- 支持保存每期版本、从项目重新载入编辑草稿，并生成小组登记表；当前保存入口
  已完成，重新载入和登记表仍在路线图中；
