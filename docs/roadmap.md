...
- 完整的 RuleSet 可视化编辑器：当前常用 hard rule、命名小组和已实现 soft rule 已有
  表单，custom RuleSet JSON 也已提供字段级诊断；仍需覆盖复杂关系的批量编辑、全部
  hard rule 组合和更完整的逐项修复建议；
- 轮换方案的连续编辑：React 工作台已经可以保存所有期次的 snapshot、锁定状态、
  ...
  规则以及历史/输出引用是否仍然可用。后续只需补充更细的跨期时间线和操作筛选；
...
- 迁移向导收尾：当前已显示字段级变化、迁移前后校验、备份位置和回退提示，并可在
  React 项目工作台一键恢复原文件；仍需在一次迁移多个项目文件时检查引用关系；
...
输出继续编辑；基础逐条操作时间线已经可以展开查看，后续可补充跨期筛选和更详细的时间标记。
...
- 支持保存每期版本、从项目重新载入编辑草稿，并生成 HTML/CSV 小组登记表；登记表会
  ...
  以及成员差异预览均已完成，后续可补充跨期筛选、时间标记和更丰富的登记字段；
