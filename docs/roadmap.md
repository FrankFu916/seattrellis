...
v1.5 的重点是消除首次使用门槛，并把 v1.4 已具备的编辑能力整理成完整工作流。
现有 Streamlit 页面继续承担过渡入口，但不再承载新的复杂交互架构。

### 主要功能

- 新首页提供“新建班级”“打开班级”“体验示例”和“恢复备份”；
- 新班级向导依次完成班级名称、学生名单、教室和排座目标；
- 导入常见 Excel/CSV 时自动识别姓名列，并在不确定时要求确认；
- 提供标准教室生成器，按排数、每排座位和过道生成 layout，无需手写 JSON；
- 默认提供“日常轮换”“公平打散”“小组协作”和“自定义”目标卡；
- 自动考虑名单中的视力、身高和历史信息，并用简短摘要说明；
...
- 只有一份常见格式名单、没有 layout 或 rules 文件的新用户，可以在五分钟内
  生成并打印座位表；
- 默认流程不出现 JSON、seed、backend、snapshot 或 candidate 等技术术语；
- 输入缺失或有冲突时，页面给出一个明确的下一步操作，而不是只显示错误详情；
- 浏览器 E2E 覆盖“导入名单 → 标准教室 → 生成 → 调整 → 导出”；
- 旧 CLI、service API、Project 文件和现有 Web 工作流继续可用。
...
- 新规则具备求解、验证、评分、文档和契约测试，不再发布 model-only 功能；
