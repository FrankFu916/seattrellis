...
历史轮换、近期邻座、成绩位置/分布和互助搭档；`groups` 的组内相邻/分离语义也已
接入统一硬约束编译。完整的 RuleSet 编辑器、`cooling` 软规则语义和更细的迁移向导
仍待后续迭代；历史比较已经支持匿名逐项座位变化预览。
...
- 新规则具备求解、验证、评分、文档和契约测试；仍为 model-only 的 `cooling` 不会在
  普通设置中伪装成已生效功能；
