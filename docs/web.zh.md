...
面板支持检查项目中的敏感字段、下载 `.seattrellis.zip` 备份，以及把上传的项目包
恢复到本机目录。浏览器上传和 CLI 使用同一套路径安全与 manifest 校验。教室编辑器
已经可以在普通流程中修改网格、走廊、讲台和空位；学生和规则编辑器、历史比较与
原地恢复仍在后续路线中。
