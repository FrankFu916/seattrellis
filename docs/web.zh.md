...
求解后展开“人工调整”，选择两名已入座学生即可交换座位。每次交换都会立即复核
当前 `RuleSet` 的 hard constraints，并在结果区明确显示通过状态或违规数量。操作
按顺序写入草稿，可以连续撤销和重做；当前草稿可直接导出，也可以作为“锁定与
局部重排”的输入继续自动修复。
...
- 尚未提供拖拽 layout 编辑。
- 当前交换与局部重排通过表单选择学生和座位，尚未提供座位图上的框选或拖拽。
