...
前后座位变化。教室的常见不规则布局和座位要求可以直接在普通流程中设置；需要完整
规则 JSON、候选数量、随机种子、时间限制或求解后端时，再展开生成页的“高级设置”。
详细规则面板只编辑已经接入求解、验证和评分的规则；`groups` 已支持通过 JSON
配置组内必须相邻或不得相邻，`cooling` 仍会明确提示为 model-only。
